...
public class ProjectionDependentWeights extends Weights<CoordinateSet> {
	...
	public static void main(String[] args) throws IOException {
		ProjectionDependentWeights myWeights = new ProjectionDependentWeights();
		OrderDependentWeights orderWeights = new OrderDependentWeights();
		orderWeights.add(4,0.33);
		orderWeights.add(1,0.23);

		
		myWeights.setOrderDependentWeights(orderWeights.getComparableWeights());
		myWeights.addOrderDependentWeight(2, 0.7);
		
		CoordinateSetLong baseCoordSet = new CoordinateSetLong(7);
		List<CoordinateSet> coordList = baseCoordSet.subsetsNotEmpty(2);
		coordList.add(new CoordinateSetLong(1101));
		
		for(CoordinateSet coords : coordList)
			myWeights.add(coords, 1.0/((double) coords.cardinality()));
