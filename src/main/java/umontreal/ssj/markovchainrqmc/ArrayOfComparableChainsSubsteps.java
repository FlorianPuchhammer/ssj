...
public class ArrayOfComparableChainsSubsteps<T extends MarkovChainComparableWithSubsteps> extends ArrayOfComparableChains<T> {

	List<MultiDimSort<MultiDim>> sortList;
	
	

	public ArrayOfComparableChainsSubsteps(T baseChain, List<MultiDimSort<MultiDim>> sortList) {
		super(baseChain);
		this.sortList = sortList;
//		this.baseChain.numSubsteps = numSubsteps;
	}

	public ArrayOfComparableChainsSubsteps(T baseChain, PointSetRandomization rand,
			List<MultiDimSort<MultiDim>> sortList, int numSubsteps) {
		super(baseChain);
		this.sortList = sortList;
...
//		this.baseChain.numSubsteps = numSubsteps;
	...
	public double simulArrayRQMC(PointSet p, PointSetRandomization rand, int sortCoordPts, int numSteps)
			throws IOException {
...
//				System.out.println("step " + step + "loaded!"); //test
			
			for (int curSubstep = 0; curSubstep < baseChain.numSubsteps; curSubstep++) {

				sort = sortList.get(curSubstep);
...
//				System.out.println("Chain " + step + "loaded!"); //test
				
//					System.out.println("Substep " + curSubstep + "loaded!"); //test
//					System.out.println("current substep:\t" + curSubstep);
					
					if (mc.hasStopped()) {
						numNotStopped--;
					} else {
						stream.setCurCoordIndex(sortCoordPts); // Skip first sortCoordPts coord.
						mc.nextStep(stream, curSubstep); // simulate next step of the chain.
	...
	// Same as usual, but removed sort
	public void simulReplicatesArrayRQMC(PointSet p, PointSetRandomization rand, int sortCoordPts, int numSteps, int m,
			Tally statReps) throws IOException {
	...
	public String testVarianceRateFormat(RQMCPointSet[] rqmcPts, int sortCoordPts, int numSteps, int m, double varMC,
			String filenamePlot, String methodLabel) throws IOException {
		label = methodLabel;
		...
		for (int i = 0; i < numSets; ++i) {
			...
			str.append("n = " + n + "\n");
			simulReplicatesArrayRQMC(rqmcPts[i].getPointSet(), rqmcPts[i].getRandomization(), sortCoordPts, numSteps, m,
					statPerf);
			logn[i] = Num.log2(n);
	...
	public String testVarianceRateFormat(PointSet[] pointSets, PointSetRandomization rand, int sortCoordPts,
			int numSteps, int m, double varMC, String filenamePlot, String methodLabel) throws IOException {
		label = methodLabel;
