...
import umontreal.ssj.util.sort.NeuralNetworkSortFlo;

public class ArrayOfComparableChainsNN<T extends MarkovChainComparable> extends ArrayOfComparableChains<T>{
	
	String[] fileNamesNN;
	
	public ArrayOfComparableChainsNN(T baseChain,String[] fileNamesNN) {
		super(baseChain);
		this.fileNamesNN = fileNamesNN;
	}
	
	public ArrayOfComparableChainsNN(T baseChain, PointSetRandomization rand, String [] fileNamesNN) {
		super(baseChain);
	...
	public double simulArrayRQMC(PointSet p, PointSetRandomization rand,  int sortCoordPts,
			double numSteps) throws IOException {
	
		initialStates();
		int numNotStopped = n;
		int dim = chains[0].stateDim;
		int step = 0;
		NeuralNetworkSortFlo sort;
		while (step < numSteps && numNotStopped > 0) {
			sort = new NeuralNetworkSortFlo(fileNamesNN[step],dim);
...
//				sort.sort((MultiDim[]) chains);
	...
	public void simulReplicatesArrayRQMC(PointSet p, PointSetRandomization rand, int sortCoordPts,
			double numSteps, int m, Tally statReps) throws IOException {
	...
	public String testVarianceRateFormat(PointSet[] pointSets, PointSetRandomization rand, 
			int sortCoordPts, double numSteps, int m, double varMC, String filenamePlot, String methodLabel) throws IOException {
		label = methodLabel;
