 ...
 * 
 */
public class AsianOptionVars implements MonteCarloModelDoubleArray{
	...
	double[] path, perf; // Sample path of the process.
	 ...
	 * 
	 */
	public AsianOptionVars(double r, int d, double[] obsTimes, double strike) {
		this.d = d;
		this.obsTimes = new double[d + 1];
		this.path= new double[d+1];
		this.perf= new double[d];
		for (int j = 0; j <= d; j++)
        ...
        for (int j = 0; j < d; j++) {
            double dt     = obsTimes[j+1] - obsTimes[j];
            mudt[j]       = 0.0;
            sigmasqrdt[j] = 1.0 * Math.sqrt (dt);
        }
        //B[0] = 0.0;
	}
	...
	public AsianOptionVars(StochasticProcess sp, double r, int d,
	 ...
	 */
	public AsianOptionVars(double r, int d, double T1, double T, double strike) {
		this.d = d;
	...
	public void simulate(RandomStream stream) {
		...
		/*p= B[0];
		for (int j = 0; j < d; j++) {
            p += mudt[j] + sigmasqrdt[j] * gen.nextDouble();
            B[j + 1] = p;
        }
		double sum;
		int d=0;
