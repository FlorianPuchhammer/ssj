...
class LacZLacYwithoutnegativepopulationComparable extends MarkovChainComparable  implements MultiDim01, MultiDim {
	...
	public LacZLacYwithoutnegativepopulationComparable(int K, int N, double[] c, double []  X0 , double[][] D, double tau, int[][] Reactant, int [][] Product,int[][] nuReactant , int[][] nuProduct,int T, RealsTo01Map map, double epsilon) {
		this.K = K;
	...
	double calculatePropensity( int reaction, double [] X, double volume){
		 double re = c[reaction];
	double vol = volume;
	if (volume>0)
		re = getConstantFromDeterministicRateConstant(re, reaction, vol);
	return re;
	}
	public double getConstantFromDeterministicRateConstant(double k, int reaction, double V) {
		double re = k/Math.pow(V,Reactant[reaction].length-1);
		for (int i=0; i<reactantHistosKeys[reaction].length; i++) { //int r : reactantHistos[reaction].keySet()) {
			int freq = reactantHistosVals[reaction][i];
			re*=faculty(freq);
		}
	...
	public double[] calculatePropensity( double volume , double[] X) {
		a_sum = 0;		
		
		for( int reaction =0; reaction<K; reaction++)
		a[reaction] =calculatePropensity(reaction, X, volume) ;
	    for (int i=0; i<a.length; i++) {
 ...
 public void performSSA(int numStep, RandomStream stream){
	 int index;
	 double t=0;
	double  genTime = numStep;
	 X =X0;
	 while (t< numStep){
		/* X[1]  = 35   * (1 + t/genTime);
	     X[9]  = 350 * (1 + t/genTime);*/
	     X[1]  = NormalDist.inverseF(35 * (1 + step/genTime), 3.5, stream.nextDouble()); 
	     X[9]  = NormalDist.inverseF(350 * (1 + step/genTime), 35, stream.nextDouble()); 
		 double volume = 1. + step/genTime;
			a =calculatePropensity(volume, X);
 ...
 for ( int i=0; i<N; i++)
	...
	*/
	...
	public int compareTo(MarkovChainComparable m, int i) {
		if (!(m instanceof LacZLacYwithoutnegativepopulationComparable)) {
			throw new IllegalArgumentException("Can't compare a " + "Biology example with other types of Markov chains.");
		}
		double  mx;
		
			mx = ((LacZLacYwithoutnegativepopulationComparable) m).X[i];
 ...
 // Returns coordinate j of the state mapped to (0,1).
    public double getCoordinate (int j) {
 	...
 	/*  //zvalue = (X[j]- X0[j])/(Math.sqrt(2*a[j]*step*X0[j])) ;          
        ...
        case 0:   
        	zvalue = (X[j]- X0[j]+c[0]*X0[0]*X0[1]*step-c[1]*X0[2]*step-c[3]*X0[3]*step)/(Math.sqrt(c[0]*X0[0]*X0[1]*step+c[1]*X0[2]*step+c[3]*X0[3]*step));
            return NormalDist.cdf01 (zvalue);
        case 1:   
        	zvalue = (X[j]- X0[j]+c[0]*X0[0]*X0[1]*step-c[1]*X0[2]*step-c[6]*X0[8]*step)/(Math.sqrt(c[0]*X0[0]*X0[1]*step+c[1]*X0[2]*step+c[6]*X0[8]*step));
            return NormalDist.cdf01 (zvalue);
        case 2:  
        	zvalue = (X[j]- X0[j]-c[0]*X0[0]*X0[1]*step+c[1]*X0[2]*step+c[2]*X0[2]*step)/(Math.sqrt(c[0]*X0[0]*X0[1]*step+c[1]*X0[2]*step+c[2]*X0[2]*step));
            return NormalDist.cdf01 (zvalue);
        case 3:  
        	zvalue = (X[j]- X0[j]-c[2]*X0[2]*step+c[3]*X0[3]*step)/(Math.sqrt(c[2]*X0[2]*step+c[3]*X0[3]*step));
            return NormalDist.cdf01 (zvalue);
        case 4:  
        	zvalue = (X[j]- X0[j]-c[3]*X0[3]*step+c[7]*X0[9]*X0[4]*step-c[8]*X0[10]*step-c[11]*X0[10]*step+c[17]*X0[4]*step)/(Math.sqrt(c[3]*X0[3]*step+c[7]*X0[9]*X0[4]*step+c[8]*X0[10]*step+c[11]*X0[10]*step+c[17]*X0[4]*step));
            return NormalDist.cdf01 (zvalue);
        case 5:  
        	zvalue = (X[j]- X0[j]-c[3]*X0[3]*step+c[4]*X0[5]*step)/(Math.sqrt(c[3]*X0[3]*step+c[4]*X0[5]*step));
            return NormalDist.cdf01 (zvalue);
        case 6:  
        	zvalue = (X[j]- X0[j]-c[4]*X0[5]*step+c[5]*X0[6]*step)/(Math.sqrt(c[4]*X0[5]*step+c[5]*X0[6]*step));
            return NormalDist.cdf01 (zvalue);
        case 7:  
        	zvalue = (X[j]- X0[j]-c[5]*X0[6]*step+c[9]*X0[9]*X0[7]*step+c[18]*X0[7]*step)/(Math.sqrt(c[5]*X0[6]*step+c[9]*X0[9]*X0[7]*step+c[18]*X0[7]*step));
            return NormalDist.cdf01 (zvalue);
        case 8:  
        	zvalue = (X[j]- X0[j]-c[5]*X0[6]*step+c[6]*X0[8]*step)/(Math.sqrt(c[5]*X0[6]*step+c[6]*X0[8]*step));
            return NormalDist.cdf01 (zvalue);
        case 9:  
        	zvalue = (X[j]- X0[j]+c[7]*X0[9]*X0[4]*step-c[8]*X0[10]*step+c[9]*X0[9]*X0[7]*step-c[10]*X0[11]*step)/(Math.sqrt(c[7]*X0[9]*X0[4]*step-c[8]*X0[10]*step+c[9]*X0[9]*X0[7]*step-c[10]*X0[11]*step));
            return NormalDist.cdf01 (zvalue);
        case 10:  
        	zvalue = (X[j]- X0[j]-c[7]*X0[9]*X0[4]*step+c[8]*X0[10]*step+c[11]*X0[10]*step)/(Math.sqrt(c[7]*X0[9]*X0[4]*step+c[8]*X0[10]*step+c[11]*X0[10]*step));
            return NormalDist.cdf01 (zvalue);
        case 11:  
        	zvalue = (X[j]- X0[j]-c[9]*X0[9]*X0[7]*step+c[10]*X0[11]*step+c[12]*X0[11]*step)/(Math.sqrt(c[9]*X0[9]*X0[7]*step+c[10]*X0[11]*step+c[12]*X0[11]*step));
            return NormalDist.cdf01 (zvalue);
        case 12:  
        	zvalue = (X[j]- X0[j]-c[11]*X0[10]*step+c[13]*X0[12]*step)/(Math.sqrt(c[11]*X0[10]*step+c[13]*X0[12]*step));
            return NormalDist.cdf01 (zvalue);
        case 13:  
        	zvalue = (X[j]- X0[j]-c[12]*X0[11]*step+c[14]*X0[13]*step)/(Math.sqrt(c[12]*X0[11]*step+c[14]*X0[13]*step));
            return NormalDist.cdf01 (zvalue);
        case 14:  
        	zvalue = (X[j]- X0[j]-c[13]*X0[12]*step+c[15]*X0[14]*step+c[19]*X0[14]*X0[20]*step-c[20]*X0[21]*step)/(Math.sqrt(c[13]*X0[12]*step+c[15]*X0[14]*step+c[19]*X0[14]*X0[20]*step+c[20]*X0[21]*step));
            return NormalDist.cdf01 (zvalue);
        case 15:  
        	zvalue = (X[j]- X0[j]-c[14]*X0[13]*step+c[16]*X0[15]*step)/(Math.sqrt(c[14]*X0[13]*step+c[16]*X0[15]*step));
            return NormalDist.cdf01 (zvalue);
        case 16:  
        	zvalue = (X[j]- X0[j]-c[15]*X0[14]*step)/(Math.sqrt(c[15]*X0[14]*step));
            return NormalDist.cdf01 (zvalue);
        case 17:  
        	zvalue = (X[j]- X0[j]-c[16]*X0[15]*step)/(Math.sqrt(c[16]*X0[15]*step));
            return NormalDist.cdf01 (zvalue);
        case 18:  
        	zvalue = (X[j]- X0[j]-c[17]*X0[4]*step)/(Math.sqrt(c[17]*X0[4]*step));
            return NormalDist.cdf01 (zvalue);
        case 19:  
        	zvalue = (X[j]- X0[j]-c[18]*X0[7]*step)/(Math.sqrt(c[18]*X0[7]*step));
            return NormalDist.cdf01 (zvalue);
        case 20:  
        	zvalue = (X[j]- X0[j]+c[19]*X0[14]*X0[20]*step-c[21]*X0[15]*step)/(Math.sqrt(c[19]*X0[14]*X0[20]*step+c[21]*X0[15]*step));
            return NormalDist.cdf01 (zvalue);
        case 21:  
        	zvalue = (X[j]- X0[j]-c[19]*X0[14]*X0[20]*step+c[20]*X0[21]*step)/(Math.sqrt(c[19]*X0[14]*X0[20]*step+c[20]*X0[21]*step));
            return NormalDist.cdf01 (zvalue);
        case 22:  
        	zvalue = (X[j]- X0[j]-c[20]*X0[21]*step)/(Math.sqrt(c[20]*X0[21]*step));
            return NormalDist.cdf01 (zvalue);
        default:
