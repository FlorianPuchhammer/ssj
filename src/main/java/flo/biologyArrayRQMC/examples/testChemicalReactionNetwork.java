...
//		int[] N = { 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576 }; // n from 8
		...
		int sortCoordPts = 1;
		MultiDimSort sortPointSet = new SchloeglSystemSort(); //set here if sortCoordPts>1
...
//		listP.add(rqmcPts);
		...
		for (RQMCPointSet[] ptSeries : listP) {
			...
			str = (chain.testVarianceRateFormat(ptSeries, sortPointSet,sortedCoords, model.numSteps, m, 
					varMC,modelDescription + label,
					label));
			System.out.println(str);
			sb.append(str + "\n");
		}
