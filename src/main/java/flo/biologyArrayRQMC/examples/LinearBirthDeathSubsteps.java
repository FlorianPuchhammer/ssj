...
public class LinearBirthDeathSubsteps extends ChemicalReactionNetworkSubsteps implements MultiDim01 {

	public LinearBirthDeathSubsteps(double[] c, double[] X0, double tau, double T) {
		this.c = c;
		this.X0 = X0;
		this.tau = tau;
		this.T = T;
	...
	public String toString() {
		...
		sb.append("LinearBirthDeath with Substeps:\n");
		...
		sb.append("T =\t" + T + "\n");
		sb.append("tau =\t" + tau + "\n");
		sb.append("steps =\t" + numSteps/K + "\n");
		sb.append("substeps =\t" + numSubsteps + "\n");
		sb.append("----------------------------------------------\n\n");
	...
	public int compareTo(MarkovChainComparable m, int i) {
		if (!(m instanceof LinearBirthDeathSubsteps)) {
			throw new IllegalArgumentException("Can't compare LinearBirthDeathSubsteps with other types of Markov chains.");
		}
		double mx;

		mx = ((LinearBirthDeathSubsteps) m).X[i];
