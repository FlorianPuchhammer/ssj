...
public class AsianOptionTest extends ArrayOfComparableChains<AsianOptionComparable2> {

	public AsianOptionTest (AsianOptionComparable2 baseChain) {
		super(baseChain);
...
//				131072, 262144, 524288, 1048576 }; // logn from 8 to 20.
		...
		for (s = 0; s < numSets; ++s) {
		  ...
		  System.out.println (testVarianceRateFormat (pointSets, randShift, sort, 0, 
					 numSteps, m, varMC, "Asian-var-IndependentPoint", "Independent points")); 
		...
		System.out.println (testVarianceRateFormat (pointSets, randShift, sort, sortCoordPts, 
				 numSteps, m, varMC, "Asian-var-Stratification", "Stratification")); */
		...
		System.out.println (testVarianceRateFormat (pointSets, randDigital, sort, 0, 
			 numSteps, m, varMC, "Asian-var-sobol", "Sobol+LMS")); 
		/*
		...
		// Sobol NUS
		 ...
		 for (s = 0; s < numSets; ++s){
		  ...
		  System.out.println (testVarianceRateFormat (pointSets, randShift, sort, 0,  
					 numSteps, m, varMC, "Asian-var-korobov", "Korobov "));
		// Korobov +baker
		for (s = 0; s < numSets; ++s) {
			if (sortCoordPts == 1)
	    ...
	    System.out.println (testVarianceRateFormat (pointSets, randShift, sort, 0,  
				 numSteps, m, varMC,  "Asian-var-korobov-Baker", 
				 "Korobov + Baker"));*/
	}
	...
	//	int numSets = 10; // Number of point sets to try.
		...
		AsianOptionComparable2 asian = new AsianOptionComparable2 (r, d, t1, T, K,
				s0, sigma);
		AsianOptionTest test = new AsianOptionTest (asian);    // This is the array of comparable chains.
		...
		//sort02 = new NeuralNetworkSort(2);
		test.testMethods(asian, sort02, 1, d, m, numSets);
