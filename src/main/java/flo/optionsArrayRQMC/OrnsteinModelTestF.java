...
public class OrnsteinModelTestF extends ArrayOfComparableChains<OrnsteinModelComparableF> {

	public OrnsteinModelTestF (OrnsteinModelComparableF baseChain) {
		super(baseChain);
        //  this.baseChain = baseChain;
	}
	public void testMethods (OrnsteinModelComparableF asian,
...
int mink = 16;  
		...
		String outFile = "OrnsteinAsianF" + sor +".txt";
		 ...
		 * //pointSets[s] = new IndependentPointsCached(N[s], stateDim+1); //}
		 * // testVarianceRate (pointSets, rand, asian.d, m, varMC,
		 * "Asian-var-indep", "Independent points");
		 *
...
//Independent points (Monte Carlo) 
		...
		str =testVarianceRateFormat(pointSets, randShift, sort, sortCoordPts, numSteps, m, varMC,
				sor + "OrnsteinAsianF-var-indep", "Independent points");
		...
		// Stratification
				...
				str =testVarianceRateFormat(pointSets, randShift, sort, sortCoordPts, numSteps, m, varMC,
						sor + "OrnsteinAsianF-var-Stratification", " Stratification");
				System.out.println(str);
				sb.append(str + "\n");
		...
		for (s = 0; s < numSets; ++s) {
			if (sortCoordPts == 1)
				pointSets[s] = new SobolSequence(s + 8, 31, 4);
			else
				// The points are sorted here, but only once.
			    pointSets[s] = new SortedAndCutPointSet (new SobolSequence(s + 8, 31, 5), sort);
		}
		...
		System.out.println (testVarianceRateFormat (pointSets, randDigital, sort, 0,
			 numSteps, m, varMC, null, "\npoint sets: Sobol + LMS\n"));
*/
		
//		// Sobol + Baker
//		for (s = 0; s < numSets; ++s) {
//			pointSets[s] = new BakerTransformedPointSet (new SobolSequence(s + 8,
//					31, dimState));
//		}
//		testVarianceRate(pointSets, rand2, asian.d, m, varMC,
//				"Asian-var-sobol-baker", "Sobol + Baker");
		...
		// Sobol + LMS
				...
				str =testVarianceRateFormat(pointSets, randDigital, sort, 0, numSteps, m, varMC,
						sor + "OrnsteinAsianF-var-Sobol + LMS", "Sobol + LMS");
				System.out.println(str);
				sb.append(str + "\n");
				...
				str =testVarianceRateFormat(pointSets, randDigital, sort, 0, numSteps, m, varMC,
						sor + "OrnsteinAsianF-var-Sobol + LMS + Baker", "Sobol + LMS+Baker");
				System.out.println(str);
				sb.append(str + "\n");
		...
		str =testVarianceRateFormat (pointSets, randNUS, sort, 0,
				 numSteps, m, varMC, sor + "OrnsteinAsianF-var-Sobol + NUS", "Sobol + NUS");
		System.out.println (str);
		sb.append(str + "\n");
	 ...
	 // Korobov
	 		...
	 		str =testVarianceRateFormat(pointSets, randShift, sort, 0, numSteps, m, varMC,
	 				"OrnsteinAsianF-var-korobovstandard", " Korobovstandard");
	 		System.out.println(str);
	 		sb.append(str + "\n");
	 		// Korobov + baker
	 		...
	 		str =testVarianceRateFormat(pointSets, randShift, sort, 0, numSteps, m, varMC,
	 				"OrnsteinAsianF-var-korobovstandard-Baker", " Korobovstandard + Baker");
	 		System.out.println(str);
	 		sb.append(str + "\n");
	 	// Korobov
	 		for (s = 0; s < numSets; ++s) {
	 		...
	 		str =testVarianceRateFormat(pointSets, randShift, sort, 0, numSteps, m, varMC,
	 				"OrnsteinAsianF-var-korobov", " Korobov");
	 		System.out.println(str);
	 		sb.append(str + "\n");
	 		// Korobov + baker
	 		...
	 		str =testVarianceRateFormat(pointSets, randShift, sort, 0, numSteps, m, varMC,
	 				"OrnsteinAsianF-var-korobov-Baker", " Korobov + Baker");
	 		System.out.println(str);
	 		sb.append(str + "\n");
	 	...
	 	// lattice
			for (s = 0; s < numSets; ++s) {
				if (sortCoordPts == 1)
					pointSets[s] = new Rank1Lattice(N[s], a, 1 + 2);
				else
					pointSets[s] = new SortedAndCutPointSet(new Rank1Lattice(N[s], a, sortCoordPts + 2), sort);
			}
			str =testVarianceRateFormat(pointSets, randShift, sort, 0, numSteps, m, varMC,
					sor + "OrnsteinAsianF-var-Lattice", "Lattice");
			System.out.println(str);
			sb.append(str + "\n");
			...
			for (s = 0; s < numSets; ++s) {
				if (sortCoordPts == 1)
					pointSets[s] = new BakerTransformedPointSet(new Rank1Lattice(N[s], a, 1 + 2));
				else
					// The points are sorted here, but only once.
					pointSets[s] = new SortedAndCutPointSet(
							new BakerTransformedPointSet(new Rank1Lattice(N[s], a, sortCoordPts + 2)), sort);
			}
			str =testVarianceRateFormat(pointSets, randShift, sort, 0, numSteps, m, varMC,
					sor + "OrnsteinAsianF-var-Lattice+baker", "Lattice+baker");
			System.out.println(str);
			sb.append(str + "\n");
	...
	public static void usage() {
		System.err.println("usage: OrnsteinAsianTest { split | batch | hilbertbatch | hilbert { normal | logistic <x0> <w> | logisticGC <mu> <c> } }");
		System.err.println("examples:");
		System.err.println("    OrnsteinAsianTest split");
		System.err.println("    OrnsteinAsianTest batch");
		System.err.println("    OrnsteinAsianTest hilbert normal");
		System.err.println("    OrnsteinAsianTest hilbert logistic 0.0 2.0");
	}
	...
	public static void main(String[] args) throws IOException {
		...
		OrnsteinModelComparableF.RealsTo01Map map = null;
		...
		if (sortType.equals("hilbert")) {
			...
			if (args.length == 2 && args[1].equals("normal")) {
				...
				map = new OrnsteinModelComparableF.NormalCDFMap();
			}
				...
				map = new OrnsteinModelComparableF.LogisticGCMap(mu - c, mu + c);
			}
				...
				map = new OrnsteinModelComparableF.LogisticMap(x0, w);
			}
			else {
				usage();
				System.exit(1);
		...
		OrnsteinModelComparableF hesto = new OrnsteinModelComparableF(r,sigma,T,d,alpha,b,S0,V0,K,correlation,map);
		
		OrnsteinModelTestF test = new OrnsteinModelTestF(hesto);   
		// PointSetRandomization rand = new RandomShift(new MRG32k3a());
		MultiDimSort<OrnsteinModelComparableF> sort;
		...
		if (sortType.equals("split")) {
			...
			sort = new SplitSort<OrnsteinModelComparableF> (3);
			test.testMethods (hesto, sort, 3,d, m, numSets,"split2");
		}
		else if (sortType.equals("batch")) {
			System.out.println("\n******************************************");
			System.out.println("\nsort type: BATCH SORT\n");
			sort = new BatchSort<OrnsteinModelComparableF>(batchExp);    
			test.testMethods(hesto, sort, 3, d, m, numSets,"batch2");
		}
		else if (sortType.equals("hilbertbatch")) {
			System.out.println("\n******************************************");
			System.out.println("\nsort type: HILBERT BATCH SORT\n");
			sort = new HilbertCurveBatchSort<OrnsteinModelComparableF>(batchExp, 20);
			test.testMethods(hesto, sort, 1, d, m, numSets,"hilbertbatch2");
		}
		else if (sortType.equals("hilbert")) {
			System.out.println("\n******************************************");
			System.out.println("\nsort type: HILBERT SORT\n");
			sort01 = new HilbertCurveSort(3, 12);
			test.testMethods(hesto, sort01, 1, d, m, numSets,"hilbert2"+args[1]);
		}
		else {
